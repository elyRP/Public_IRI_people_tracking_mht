...
PeopleTrackingMhtAlgNode::PeopleTrackingMhtAlgNode(void) :
  algorithm_base::IriBaseAlgorithm<PeopleTrackingMhtAlgorithm>(), tf_(ros::Duration(1.f)),
  max_dist_to_detect_people_(10.0), robot_actual_pose_x_(0.0), robot_actual_pose_y_(0.0)
{
 ...
 // this->frame_id = "ibeo1";
  ...
  // [init publishers]
  this->tracksMarkers_publisher_ = this->public_node_handle_.advertise<visualization_msgs::MarkerArray>("tracksMarkers", 1);
  this->tracks_publisher_ = this->public_node_handle_.advertise<iri_perception_msgs::detectionArray>("tracks", 1);
...
void PeopleTrackingMhtAlgNode::mainNodeThread(void)
{
 ...
 // ros::Time now    = ros::Time::now();
  ...
  if(this->detectionsReceived) // to iterate only when detection are received.
  {
    ...
    this->alg_.iteration(this->detections, this->tracks, this->predictions, this->clusters, this->odom, this->velocity, this->groups,this->local_tracker);
...
void PeopleTrackingMhtAlgNode::odom_for_medium_velocity_callback(const nav_msgs::Odometry::ConstPtr& msg) 
{ 
...
bool PeopleTrackingMhtAlgNode::tf_to_eigen_odom(const tf::Transform& tf_odom_pose_in, Eigen::Matrix4d& act_odom_matrix_tf,double dt) 
{ 
...
void PeopleTrackingMhtAlgNode::odom_callback(const nav_msgs::Odometry::ConstPtr& msg) 
{ 
  ...
  if(first_odometry==true){
    ...
    this->alg_.save_time_stamp_odometry_to_tracker_without_det_=time_odom_act;
...
//ROS_INFO( "(in odometry) time_odom_act=%f; save_time_stamp_odometry_to_tracker_without_det_= %f",time_odom_act,this->alg_.save_time_stamp_odometry_to_tracker_without_det_);
    ...
    if(!cargo_ANTS){    
    	geometry_msgs::Quaternion my_orient_Q=tf::createQuaternionMsgFromRollPitchYaw(msg->pose.pose.orientation.x,msg->pose.pose.orientation.y,msg->pose.pose.orientation.z);
    	
  		//before_theta=msg->twist.twist.angular.z;//tf::getYaw(my_orient_Q);
  		before_theta=tf::getYaw(my_orient_Q); 		
  	}else{
    ...
    tf::poseMsgToTF(odompose,tf_odom_pose);
...
/////////////////
    ...
    if(!cargo_ANTS){
    	...
    	tf::Quaternion my_orient;
    	my_orient = tf::Quaternion(msg->pose.pose.orientation.x,msg->pose.pose.orientation.y,msg->pose.pose.orientation.z, msg->pose.pose.orientation.w);
    	act_theta=tf::getYaw(my_orient);
    	//ROS_INFO("act_theta1= %f",act_theta1); 
    	geometry_msgs::Quaternion my_orient_Q=tf::createQuaternionMsgFromRollPitchYaw(msg->pose.pose.orientation.x,msg->pose.pose.orientation.y,msg->pose.pose.orientation.z);
    	
  		//act_theta=msg->twist.twist.angular.z;//tf::getYaw(my_orient_Q);
  		act_theta1=tf::getYaw(my_orient_Q); 
...
void PeopleTrackingMhtAlgNode::detections_callback(const iri_perception_msgs::detectionArray::ConstPtr& msg)
{
...
void PeopleTrackingMhtAlgNode::publishMarkers()
{
  ...
  for(unsigned int i=0;i<n_track;i++)
  {
    ...
    if(this->id_markers)
    {
      ...
      MarkerArray_msg_.markers.push_back(id_track_marker_);
    }
    ...
    if(this->vel_markers)
    {
      ...
      geometry_msgs::Quaternion myQ = tf::createQuaternionMsgFromYaw(yaw);       
 ...
 if(this->clust_markers)
 {
  ...
  {  
  	n_track = this->trackingArray_msg_.detection.size();
 		for(unsigned int i=0;i<n_track;i++)
		{
			visualization_msgs::Marker track_path_;
			track_path_.header = this->trackingArray_msg_.header;
  		track_path_.ns     = "track_path";
  		track_path_.type   = visualization_msgs::Marker::LINE_STRIP;
  		track_path_.action = visualization_msgs::Marker::ADD;
	  	...
	  	//fill_my_marker_track_path(actual_id);  			  		
			...
			track_path_.lifetime = ros::Duration(0.1f);
...
void PeopleTrackingMhtAlgNode::fill_my_covariance_marker_track( unsigned int i )
{
  ...
  cov_track_marker_.pose.orientation = tf::createQuaternionMsgFromYaw(angle);
}
...
void PeopleTrackingMhtAlgNode::fill_my_covariance_marker_track_head( unsigned int i )
{
  ...
  double angle = atan2( eig.eigenvectors()(1,0), eig.eigenvectors()(0,0) );
  cov_track_marker_head_.pose.orientation = tf::createQuaternionMsgFromYaw(angle);
}
...
void PeopleTrackingMhtAlgNode::fill_my_covariance_marker_track_arm1( unsigned int i )
{
  ...
  {
    cov_track_marker_arm1_.pose.orientation = tf::createQuaternionMsgFromYaw(0.0);
  }
  else
  {
  	double yaw = std::atan2(this->trackingArray_msg_.detection[i].velocity.y,this->trackingArray_msg_.detection[i].velocity.x);
...
//  double angle = atan2( eig.eigenvectors()(1,0), eig.eigenvectors()(0,0) );
  ...
  {
  	cov_track_marker_arm2_.pose.orientation = tf::createQuaternionMsgFromYaw(0.0);       
	}
  else
  {
		double yaw = std::atan2(this->trackingArray_msg_.detection[i].velocity.y,this->trackingArray_msg_.detection[i].velocity.x);
...
void PeopleTrackingMhtAlgNode::fill_my_covariance_marker_prediction( unsigned int i )
{
  ...
  double angle = atan2( eig.eigenvectors()(1,0), eig.eigenvectors()(0,0) );
  cov_prediction_marker_.pose.orientation = tf::createQuaternionMsgFromYaw(angle);
}
...
void PeopleTrackingMhtAlgNode::fill_my_marker_cluster( unsigned int i )
{
  ...
  double angle = atan2( eig.eigenvectors()(1,0), eig.eigenvectors()(0,0) );
  cluster_marker_.pose.orientation = tf::createQuaternionMsgFromYaw(angle);
}
...
void PeopleTrackingMhtAlgNode::fill_my_marker_group( unsigned int i )
{
  ...
  group_marker_.lifetime = ros::Duration(0.1f);
}
...
void PeopleTrackingMhtAlgNode::ini_markers()
{
  ...
  id_track_marker_.type   = visualization_msgs::Marker::TEXT_VIEW_FACING;
  id_track_marker_.action = visualization_msgs::Marker::ADD;
  ...
  id_track_marker_.lifetime = ros::Duration(0.1f);
  
  // ini id cluster markers
  id_cluster_marker_.header = this->trackingArray_msg_.header;
  id_cluster_marker_.ns     = "id_cluster";
  id_cluster_marker_.type   = visualization_msgs::Marker::TEXT_VIEW_FACING;
  id_cluster_marker_.action = visualization_msgs::Marker::ADD;
  ...
  id_cluster_marker_.lifetime = ros::Duration(0.1f);

  // ini velocity markers
  velocity_track_marker_.header = this->trackingArray_msg_.header;
  velocity_track_marker_.ns     = "vel";
  velocity_track_marker_.type   = visualization_msgs::Marker::ARROW;
  velocity_track_marker_.action = visualization_msgs::Marker::ADD;
  ...
  velocity_track_marker_.lifetime = ros::Duration(0.1f);

  // ini covariance markers TRACKS (body cov track marckers)
  cov_track_marker_.header = this->trackingArray_msg_.header;
  cov_track_marker_.ns     = "cov_track";  
  if(cargo_ANTS){
  	cov_track_marker_.type   = visualization_msgs::Marker::CUBE;
  }else{
  	cov_track_marker_.type   = visualization_msgs::Marker::CYLINDER;
  }  
  cov_track_marker_.action = visualization_msgs::Marker::ADD;
  cov_track_marker_.color.r = 0.0;
  cov_track_marker_.color.g = 1.0;
  cov_track_marker_.color.b = 0.0;
  cov_track_marker_.color.a = 0.7;
  cov_track_marker_.lifetime = ros::Duration(0.1f);
  
	// ini covariance markers TRACKS (head cov track marckers)
  cov_track_marker_head_.header = this->trackingArray_msg_.header;
  cov_track_marker_head_.ns     = "cov_track_head";
  cov_track_marker_head_.type   = visualization_msgs::Marker::SPHERE;
  cov_track_marker_head_.action = visualization_msgs::Marker::ADD;
  ...
  cov_track_marker_head_.lifetime = ros::Duration(0.1f);
  
  // ini covariance markers TRACKS (arm1 cov track marckers)
  cov_track_marker_arm1_.header = this->trackingArray_msg_.header;
  cov_track_marker_arm1_.ns     = "cov_track_arm1";
  cov_track_marker_arm1_.type   = visualization_msgs::Marker::ARROW;
  cov_track_marker_arm1_.action = visualization_msgs::Marker::ADD;
  ...
  cov_track_marker_arm1_.lifetime = ros::Duration(0.1f);
  
  // ini covariance markers TRACKS (arm2 cov track marckers)
  cov_track_marker_arm2_.header = this->trackingArray_msg_.header;
  cov_track_marker_arm2_.ns     = "cov_track_arm2";
  cov_track_marker_arm2_.type   = visualization_msgs::Marker::ARROW;
  cov_track_marker_arm2_.action = visualization_msgs::Marker::ADD;
  ...
  cov_track_marker_arm2_.lifetime = ros::Duration(0.1f);
   
  // ini covariance markers DETECTIONS
  cov_detection_marker_.header = this->trackingArray_msg_.header;
  cov_detection_marker_.ns     = "cov_detection";
  cov_detection_marker_.type   = visualization_msgs::Marker::CYLINDER;
  cov_detection_marker_.action = visualization_msgs::Marker::ADD;
  ...
  cov_detection_marker_.lifetime = ros::Duration(0.1f);

  // ini covariance markers PREDICTIONS
  cov_prediction_marker_.header = this->trackingArray_msg_.header;
  cov_prediction_marker_.ns     = "cov_prediction";
  cov_prediction_marker_.type   = visualization_msgs::Marker::CYLINDER;
  cov_prediction_marker_.action = visualization_msgs::Marker::ADD;
  ...
  cov_prediction_marker_.lifetime = ros::Duration(0.1f);
   
  // ini markers CLUSTERS
  cluster_marker_.header = this->trackingArray_msg_.header;
  cluster_marker_.ns     = "cluster_size";
  cluster_marker_.type   = visualization_msgs::Marker::CYLINDER;
  cluster_marker_.action = visualization_msgs::Marker::ADD;
  cluster_marker_.scale.z = 0.1;
  cluster_marker_.color.a = 0.2; 
  cluster_marker_.lifetime = ros::Duration(0.1f);
  
  // ini markers GRUPOS
  group_marker_.header = this->trackingArray_msg_.header;
  group_marker_.ns     = "group_size";
  group_marker_.type   = visualization_msgs::Marker::CUBE;
  group_marker_.action = visualization_msgs::Marker::ADD;
